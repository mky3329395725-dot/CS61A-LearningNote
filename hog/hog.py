...
def roll_dice(num_rolls, dice=six_sided):
    """Simulate rolling the DICE exactly NUM_ROLLS > 0 times. Return the count of
...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    ps_od = player_score % 10 # player score's one digit
    os_td = opponent_score // 10 % 10 # opponent score's ten digit
    result = abs(ps_od - os_td) * 3
    if result < 1:
        return 1
    return result
    # END PROBLEM 2
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided): # 这里写dice=six_sided是为了默认用六面公平骰子，但保留传入test专用固定骰子的接口（传参
    ...
    return roll_dice(num_rolls, dice) # 一定注意这里要把dice传进来，以便从形参传入控制使用的骰子，而不是默认用六面公平骰子
    # END PROBLEM 3
...
def num_factors(n):
    ...
    i = 2
    count = 2
    while i * i < n: # 一旦i是n的因子，则n/i一定也是，i == n/i的情况在此条件下不会被考虑
        if n % i == 0:
            count += 2
        i += 1
    if int(n**0.5) * int(n**0.5) == n:# 考虑i == n/i的情况
        count += 1
    return count
    # END PROBLEM 4
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    """
    # BEGIN PROBLEM 4
    score1 = player_score + take_turn(num_rolls, player_score, opponent_score, dice)
    score2 = sus_points(score1)
    return score2
    # END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    Ascounte that the dice always return positive outcomes.
